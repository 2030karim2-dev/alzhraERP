-- ==============================================================================
-- Migration: Inventory Race Condition Prevention + Price Validation
-- Date: 2026-08-09 | Priority: CRITICAL
-- Fixes: C3 (Over-selling), C4 (Price Validation), C9 (Race Condition)
-- ==============================================================================

-- 1. commit_sales_invoice_v2 — Atomic + Concurrency Safe + Price Validated + Idempotent
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    SELECT id INTO v_invoice_id FROM public.sales_invoices
  ...
  LOOP
    ...
    v_total_tax := v_total_tax + (v_line_total * v_item.tax_rate / 100);
  END LOOP;
  ...
  WHERE company_id = v_company_id
    AND invoice_date BETWEEN date_trunc(''year'', CURRENT_DATE) AND CURRENT_DATE + INTERVAL ''1 day'';

  -- === PHASE 3: CREATE INVOICE ===
  INSERT INTO public.sales_invoices (
    company_id, invoice_number, party_id, invoice_date, due_date,
    total_amount, total_tax, payment_type, status, notes,
    ...
    v_total_amount, v_total_tax, p_payment_type, ''posted'', p_notes,
  ...
  LOOP
    ...
    INSERT INTO public.invoice_items (
      invoice_id, product_id, quantity, unit_price, total, tax_rate, company_id
    ) VALUES (
      v_invoice_id, v_item.product_id, v_item.quantity, v_item.unit_price,
      v_line_total, v_item.tax_rate, v_company_id
    );
    ...
    INSERT INTO public.inventory_transactions (
      ...
      -v_item.quantity, ''sale'', ''sales_invoice'', v_invoice_id,
      ''Sale Invoice '' || v_invoice_number, v_user_id
    );
  END LOOP;
  ...
  IF p_party_id IS NOT NULL THEN
    ...
    SET balance = balance + (v_total_amount + v_total_tax), updated_at = now()
    WHERE id = p_party_id AND company_id = v_company_id;
  END IF;
...
COMMENT ON FUNCTION public.commit_sales_invoice_v2 IS
''Atomic sales invoice creation with:
- Tenant ownership verification (auth.uid() -> user_profiles.company_id)
- Idempotency via idempotency_key (prevents duplicate invoices)
- Stock validation with SELECT FOR UPDATE (prevents race conditions)
- Price validation (minimum 70% of sale_price)
- Automatic inventory deduction + movement recording
- Party balance update
- Full ROLLBACK on any error'';

-- Grant execute
GRANT EXECUTE ON FUNCTION public.commit_sales_invoice_v2(
  uuid, date, date, jsonb, text, text, text, numeric, text, uuid
) TO authenticated;
