-- Migration: 20260914002000_optimize_inventory_search_trgm.sql
-- Description: Optimize product search across inventory, invoices, and stocktaking.
-- 1. Add stored generated column `normalized_search_text` using `public.normalize_arabic`.
-- 2. Create high-performance GIN trgm index on `normalized_search_text`.
-- 3. Upgrade `public.search_inventory_paginated` to use the GIN index with LIKE ALL pattern matching.

-- 1. Create stored normalized column for fast trigram index matching
ALTER TABLE public.products DROP COLUMN IF EXISTS normalized_search_text CASCADE;
ALTER TABLE public.products ADD COLUMN normalized_search_text text GENERATED ALWAYS AS (
...
-- 3. Upgrade search_inventory_paginated RPC
...
BEGIN
  ...
  -- 1. حساب إجمالي المطابقات فائق السرعة عبر الفهرس
  IF cardinality(v_tokens) = 0 OR v_patterns IS NULL THEN
  ...
  WHERE p.company_id = p_company_id
    ...
    AND (p_is_core IS NULL OR p.is_core = p_is_core)
    AND (
      v_patterns IS NULL OR
      p.normalized_search_text LIKE ALL (v_patterns)
    )
  ORDER BY
