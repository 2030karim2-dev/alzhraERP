...
export const useAdvancedTabs = ({
  ...
  const animationConfig = useMemo(
    () => ({ ...DEFAULT_ANIMATION_CONFIG, ...animation }),
    [animation]
  );
  ...
  // ── ARIA Attributes ────────────────────────────────────
  const getTabAttributes = useCallback(
    (tabId: string, _index: number) => {
      const tab = tabs.find(t => t.id === tabId);
  ...
  return {
    ...
    containerRef,
    handleTabClick,
    handleTabMouseEnter: useCallback((_tabId: string) => undefined, []),
    handleTabMouseLeave: useCallback(() => undefined, []),
    handleTabFocus,
    handleTabBlur,
    handleKeyDown,
    handleDragStart,
