...
export const useTableKeyboardNavigation = <T>({
  ...
  const getLeftRightDelta = useCallback(
  ...
  const startEditing = useCallback(
    (rowIdx: number, colIdx: number) => {
      ...
      if (col && col.isEditable && onCellUpdate && col.accessorKey) {
        ...
        setEditValue(
          row ? String((row as Record<string, unknown>)[col.accessorKey as string] ?? '') : ''
        );
  ...
  const copySelection = useCallback(() => {
    ...
    for (let r = minRow; r <= maxRowSel; r++) {
      ...
      for (let c = minCol; c <= maxColSel; c++) {
        ...
        const value =
          rowData && colDef?.accessorKey
            ? (rowData as Record<string, unknown>)[colDef.accessorKey as string]
            : '';
        rowCells.push(value);
