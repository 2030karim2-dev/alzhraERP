...
const startResize = (
    ...
    act(() => result.current.onResizeMouseDown(
        { target: handle, pageX, preventDefault: vi.fn(), stopPropagation: vi.fn() } as unknown as React.MouseEvent,
        field,
    ));
};
...
describe('useColumnResize', () => {
    ...
    it('يحفظ العروض في localStorage بعد التغيير ويعيد تحميلها', () => {
        ...
        const { result: reloaded } = renderHook(() => useColumnResize({
            storageKey: 'persist-cols',
            defaultWidths: { name: 200 },
            isRTL: false,
        }));
    ...
    it('ينظف مستمعات document ويلغي الإطار المعلّق ويعيد أنماط الجسم عند unmount', () => {
        ...
        const { handle } = makeTarget(200);
        startResize(result, handle, 100);
        act(() => fireMouseMove(150)); // جدولة إطار
        expect(rafQueue.length).toBe(1);
