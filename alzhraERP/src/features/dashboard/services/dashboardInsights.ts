...
export const resolveNetProfit = (
  ...
  const legacyNetProfit =
    sumTrialBalanceByPrefix(trialBalanceRows, '4') - sumTrialBalanceByPrefix(trialBalanceRows, '5');
...
export const calculateDashboardInsights = (data: {
  ...
  const purchasesTrend =
    olderPurchases > 0 ? ((newerPurchases - olderPurchases) / olderPurchases) * 100 : 0;

  const olderExpenses = getSum(olderHalf, 'expenses');
  const newerExpenses = getSum(newerHalf, 'expenses');
  const expensesTrend =
    olderExpenses > 0 ? ((newerExpenses - olderExpenses) / olderExpenses) * 100 : 0;
  ...
  const collectionRate =
    totalSales > 0 ? Math.min(100, Math.max(0, (collectedSales / totalSales) * 100)) : 100;
  ...
  const effectiveSalesTarget =
    salesTarget !== undefined && salesTarget > 0 ? salesTarget : Math.max(1, totalSales * 1.2);
  const salesProgress =
    totalSales > 0 ? Math.min(100, (totalSales / effectiveSalesTarget) * 100) : 0;
  insights.push({
    id: 'target',
