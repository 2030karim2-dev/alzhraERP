...
export const calculateDashboardStats = (data: {
  ...
  (invoicesData || []).forEach(inv => {
    const type = inv.type?.trim().toLowerCase();
    if (type !== 'sale' && !['return_sale', 'sale_return', 'sales_return'].includes(type || ''))
      return;
  ...
  const recentActivities = [
    ...(invoicesData || []).slice(-5).map(i => ({
      ...
      desc: formatCurrency(i.total_amount || 0, i.currency_code || 'SAR'),
      time: i.issue_date,
    ...
    ...(expensesData || []).slice(-3).map(e => ({
      ...
      desc: formatCurrency(e.amount || e.total_amount || 0, e.currency_code || 'SAR'),
      time: e.expense_date,
      color: 'rose',
    })),
  ]
