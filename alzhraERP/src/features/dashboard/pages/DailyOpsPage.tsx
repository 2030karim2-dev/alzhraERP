...
const DailySummaryCard: React.FC<DailySummaryCardProps> = ({ label, value, icon, color }) => (
  <div className="bg-[var(--app-surface)] rounded-2xl max-md:rounded-xl p-4 max-md:p-3 border border-[var(--app-border)]">
...
const DailyOpsPage: React.FC<DailyOpsPageProps> = ({ summary, className }) => {
  ...
  return (
    <div className={cn('p-4 max-md:p-3 space-y-5', className)}>
      {/* Page Title */}
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {quickActions.map((action) => (
            <button
              ...
              className="flex flex-col items-center gap-2 p-4 max-md:p-3 rounded-2xl max-md:rounded-xl transition-all duration-200 active:scale-95 hover:shadow-md"
            >
