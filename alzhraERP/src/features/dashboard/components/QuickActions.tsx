...
const QuickActions: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <div className={cn(
      "bg-[var(--app-surface)]/90 backdrop-blur-2xl border border-[var(--app-border)] rounded-2xl max-md:rounded-xl p-4 max-md:p-3 overflow-hidden relative",
      className
    )}>
      {/* Ambient glow */}
        ...
        <div className="grid grid-cols-5 gap-2">
          {actions.map((action, idx) => (
            ...
            >
              ...
              <span className="text-[10px] font-bold text-[var(--app-text-secondary)] group-hover:text-[var(--app-text)] transition-colors text-center leading-tight">
                {action.label}
              </span>
            </button>
          ))}
