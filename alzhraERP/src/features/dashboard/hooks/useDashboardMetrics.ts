...
import { type DashboardPeriod, PERIOD_LABELS } from '../types';
...
export const useDashboardMetrics = (period: DashboardPeriod = 'this_month') => {
  ...
  const { data: monthlyPerformance } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const rows = await reportService.getMonthlyPerformance(
          companyId,
  ...
  const metrics = useMemo(() => {
    ...
    const avgSales =
      salesValues.length > 0 ? salesValues.reduce((sum, v) => sum + v, 0) / salesValues.length : 0;
