...
export function mapCustomerActivity(item: CustomerActivityRow): CustomerActivity {
  ...
  if (item.assigned_to_profile?.full_name != null)
    activity.assignedToName = item.assigned_to_profile.full_name;
  ...
  if (item.created_by_profile?.full_name != null)
    activity.createdByName = item.created_by_profile.full_name;
...
export async function createActivity(
  activity: CustomerActivityFormData & { customerId: string; companyId: string }
): Promise<CustomerActivity> {
...
export async function updateActivity(
  activityId: string,
  updates: Partial<CustomerActivityFormData>
): Promise<void> {
  const updateData: {
    updated_at: string;
