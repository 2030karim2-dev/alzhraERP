...
interface SmartPurchaseResult {
  items: Array<{ name: string; suggestedQty: number; priority: string }>;
  summary: string;
}
...
export const aiService = {
  generateReportAnalysis: async (data: unknown): Promise<AiData> => {
    const result = await callAI<AiData>(
      'report_analysis',
  ...
  generateSalesForecast: async (monthlySales: AiData[]): Promise<SalesForecastResult> => {
    ...
    const result = await callAI<SalesForecastResult>(
      'sales_forecast',
  ...
  generateSmartPurchaseOrders: async (
    lowStockItems: LowStockItem[]
  ): Promise<SmartPurchaseResult> => {
    ...
    const result = await callAI<SmartPurchaseResult>(
      'smart_purchase_orders',
  ...
  analyzeInvoiceSuspicion: async (invoice: AiData): Promise<InvoiceHealthResult> => {
    ...
    const result = await callAI<InvoiceHealthResult>(
      'invoice_suspicion',
  ...
  predictStockDepletion: async (products: AiData[]): Promise<StockDepletionResult> => {
    ...
    const result = await callAI<StockDepletionResult>(
      'stock_depletion',
  ...
  parseInvoiceCommand: async (command: string): Promise<ParsedInvoiceResult> => {
    ...
    const result = await callAI<ParsedInvoiceResult>(
      'parse_invoice',
  ...
  suggestJournalEntry: async (
    description: string,
    amount: number
  ): Promise<JournalSuggestionResult> => {
    const fallback: JournalSuggestionResult = {
      debitAccount: '',
      creditAccount: '',
      explanation: '',
    };
    const result = await callAI<JournalSuggestionResult>(
      'journal_suggestion',
    ...
    if (
      result &&
      typeof result.debitAccount === 'string' &&
      typeof result.creditAccount === 'string'
    ) {
      return result;
    }
    return fallback;
