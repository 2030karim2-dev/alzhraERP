...
export const useAIPrefillStore = create<AIStoreState>((set, get) => ({
  ...
  setPendingPrefill: action => {
    set({ pendingPrefill: action });
  },
  setProductLookup: (results, action) => {
    set({ productLookupResults: results, productLookupAction: action });
  },
  clearProductLookup: () => {
    set({ productLookupResults: null, productLookupAction: null });
  },
