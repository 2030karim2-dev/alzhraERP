...
const OperationalExpensesReport: React.FC = () => {
    ...
    if (isLoading) return <div className="p-10  max-md:p-5 text-center animate-pulse text-slate-400 text-sm">جاري تحليل المصروفات التشغيلية...</div>;
    ...
    return (
        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="flex   max-md:gap-1 overflow-x-auto pb-1 -mx-1 px-1 no-scrollbar">
                {[7, 14, 30, 90].map(d => (
                    <button
                        key={d}
                        onClick={() => setDays(d)}
            ...
            {/* Chart + Table */}
            <div className="grid grid-cols-1 lg:grid-cols-3   max-md:gap-3 sm:gap-4">
                {/* Pie Chart */}
                        ...
                        <div className="w-full h-[180px] sm:h-[200px]">
                            <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={180}>
                                <PieChart>
                                    ...
                                    <Tooltip formatter={(value) => formatCurrency(Number(value) || 0)} contentStyle={{ fontSize: 11, borderRadius: 8 }} />
                ...
                <MobileCard padding="none" className={cn("lg:col-span-2", (data?.chartData.length || 0) === 0 && "lg:col-span-3")}>
                    <div className="  max-md:p-3 sm:p-4 border-b dark:border-slate-800 flex justify-between items-center bg-rose-50/50 dark:bg-rose-950/20">
                        <h4 className="font-bold text-[10px] sm:text-xs text-rose-600 uppercase flex items-center   max-md:gap-2">
