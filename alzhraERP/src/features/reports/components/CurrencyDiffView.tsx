...
const CurrencyDiffView: React.FC = () => {
  const { data, isLoading } = useCurrencyDiffs();
   ...
   if (isLoading) {
     return (
       <div className="flex flex-col items-center justify-center p-8  max-md:p-4 sm:p-12   max-md:gap-3 sm:gap-4">
   ...
   return (
     <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-20">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3   max-md:gap-4 sm:gap-6">
         <MobileCard padding="sm" className="lg:col-span-2 flex flex-col sm:flex-row items-start sm:items-center   max-md:gap-4 sm:gap-6 relative overflow-hidden group">
           ...
           <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-indigo-500/10 text-indigo-600 rounded-2xl max-md:rounded-xl sm:rounded-3xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-500 flex-shrink-0 relative z-10">
           ...
           <div className="relative z-10">
             <div className="flex items-center   max-md:gap-2 mb-1">
         ...
         <MobileCard padding="lg" className={cn("flex flex-col justify-center relative overflow-hidden group border-none shadow-2xl", totalDiff >= 0 ? 'bg-emerald-600/90 text-white' : 'bg-rose-600/50 dark:bg-rose-900/40 text-white')}>
           <div className="absolute top-0 right-0   max-md:p-3 sm:p-4 opacity-20 transform translate-x-2 -translate-y-2 sm:translate-x-4 sm:-translate-y-4 group-hover:translate-x-0 group-hover:translate-y-0 transition-transform duration-700">
           ...
           <div className="flex items-end   max-md:gap-2">
             <p dir="ltr" className="text-2xl max-md:text-lg sm:text-3xl md:text-4xl font-black font-mono text-white tracking-tighter">
               {totalDiff >= 0 ? '+' : ''}{formatCurrency(totalDiff).split(' ')[0]}
             </p>
           </div>
           <div className="mt-3 sm:mt-6 flex items-center   max-md:gap-2">
             <div className="px-2 sm:px-3 py-1 bg-white/20 backdrop-blur-md rounded-full">
               <span className="text-[10px] font-black text-white uppercase tracking-wider flex items-center   max-md:gap-1">
       ...
       <MobileCard padding="none" className="overflow-hidden">
         <div className="  max-md:p-3 sm:p-4 md:p-6 border-b border-slate-100 dark:border-slate-800/50 flex items-center   max-md:gap-2 sm:gap-3 bg-slate-50/50 dark:bg-slate-800/30">
       ...
       <MobileCard padding="sm" className="bg-amber-500/5 border border-amber-500/10 flex   max-md:gap-3 sm:gap-4 relative overflow-hidden group">
