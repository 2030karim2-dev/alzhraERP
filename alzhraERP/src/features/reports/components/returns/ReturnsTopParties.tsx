...
const ReturnsTopParties: React.FC<Props> = ({ topParties, type }) => {
    return (
        <div className="glass-panel bento-item p-10 bg-white/40 dark:bg-slate-900/40 border-none shadow-2xl backdrop-blur-3xl relative overflow-hidden group">
            ...
            {topParties.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4 max-md:gap-3">
                    {topParties.map((party, index) => (
                        <div key={index} className="group relative flex items-center justify-between p-6 bg-white/50 dark:bg-slate-800/50 rounded-2xl max-md:rounded-xl border border-slate-100 dark:border-slate-700/50 hover:border-blue-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/5 overflow-hidden">
                ...
                <div className="text-center py-16 bg-slate-50 dark:bg-slate-800/30 rounded-3xl max-md:rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
