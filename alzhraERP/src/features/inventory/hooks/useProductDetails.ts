...
export const useProductDetails = (product: Product | null) => {
  ...
  const { data: supplierData, isLoading: isSupplierLoading } = useQuery({
    ...
    queryFn: () =>
      product?.supplier_id ? inventoryApi.getSupplier(product.supplier_id) : Promise.resolve(null),
    enabled: !!product?.supplier_id,
  });

  const supplierName =
    (supplierData as { data?: { name?: string } } | null)?.data?.name ||
