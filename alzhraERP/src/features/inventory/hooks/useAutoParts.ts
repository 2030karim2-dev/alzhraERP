...
export const useCrossReferenceMutations = (productId: string) => {
  ...
  const add = useMutation({
    ...
    onError: (e: any) => {
      showToast(e.message || 'حدث خطأ أثناء الإضافة', 'error');
    },
  });
  ...
  return {
    addCrossReference: add.mutateAsync,
    removeCrossReference: remove.mutateAsync,
...
export const useKitMutations = (kitId: string) => {
  ...
  const add = useMutation({
    mutationFn: (data: { component_product_id: string; quantity: number }) => {
      if (!user?.company_id) throw new Error('Auth error');
      return inventoryApi.addKitComponent({
        ...data,
    ...
    onError: (e: any) => {
      showToast(e.message || 'حدث خطأ أثناء الربط', 'error');
    },
  });
  ...
  return {
    addKitComponent: add.mutateAsync,
    removeKitComponent: remove.mutateAsync,
    isAdding: add.isPending,
  };
};
...
export const useSupplierPriceMutations = (productId: string) => {
  ...
  const add = useMutation({
    ...
    onError: (e: any) => {
      showToast(e.message || 'فشل حفظ التسعيرة', 'error');
    },
  });
