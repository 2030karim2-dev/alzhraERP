...
export const useSmartTransferSuggestions = (
            ...
            if (suggestedQty > 0) {
              ...
              if (!dismissedSuggestions.has(key)) {
                suggestions.push({
                  ...
                  fromWarehouse:
                    source.warehouse_name || whMap.get(source.warehouse_id) || 'مستودع',
                  ...
                  priority:
                    target.quantity === 0 ? 'high' : target.quantity <= 2 ? 'medium' : 'low',
                });
              }
            }
          }
