                ...
                <div className="p-2 bg-blue-50/20 dark:bg-blue-900/10 border-b border-slate-200 dark:border-slate-800 animate-in slide-in-from-top-1">
                    {!selectedProduct ? (
                        ...
                        <div className="space-y-1.5">
                            ...
                            <button onClick={handleSave} disabled={isSaving} className="w-full bg-blue-600 text-white text-[10px] font-bold py-1 hover:bg-blue-700 transition-colors">
            ...
            <div className="flex-1 overflow-auto">
                <table className="w-full border-collapse">
                    <thead className="sticky top-0 bg-slate-50 dark:bg-slate-900 z-10 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th className="text-right px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">المنتج البديل</th>
                            <th className="text-right px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">التوافق</th>
                        ...
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan={3} className="text-center py-10 text-slate-300">
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">لا توجد بدائل</span>
                                </td>
                            </tr>
                        ) : (
                            references.map((ref) => {
                                ...
                                return (
                                    <tr key={ref.id} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                        <td className="px-4 py-1.5">
                                            ...
                                            <div className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-tight">
                                                SKU: {ref.alternative_product?.sku}
                                            </div>
                                        </td>
                                        <td className="px-4 py-1.5 whitespace-nowrap">
                                            <span className={cn("inline-flex items-center px-1.5 py-0.5 text-[10px] font-bold border border-transparent", quality.color)}>
                                                {quality.label}
                                            </span>
                                        </td>
