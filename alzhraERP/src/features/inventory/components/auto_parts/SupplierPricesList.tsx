                ...
                <div className="p-2 bg-emerald-50/20 dark:bg-emerald-900/10 border-b border-slate-200 dark:border-slate-800 animate-in slide-in-from-top-1">
                    <div className="grid grid-cols-2 gap-1.5 mb-1.5">
                        ...
                        <div className="relative">
                            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400">SR</span>
                            <input
                                type="number"
                                value={costPrice || ''}
                    ...
                    <button onClick={handleSave} disabled={isSaving} className="w-full bg-emerald-600 text-white text-[10px] font-bold py-1 hover:bg-emerald-700 transition-colors">
            ...
            <div className="flex-1 overflow-auto">
                <table className="w-full border-collapse">
                    <thead className="sticky top-0 bg-slate-50 dark:bg-slate-900 z-10 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th className="text-right px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">المورد / التاريخ</th>
                            <th className="text-right px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">التكلفة</th>
                            <th className="text-right px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">مدة الحضور</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-900">
                        {isLoading ? (
                            <tr><td colSpan={3} className="text-center py-6 text-[10px] text-slate-400 font-bold uppercase tracking-widest">جاري التحميل...</td></tr>
                        ) : !prices || prices.length === 0 ? (
                            <tr>
                                <td colSpan={3} className="text-center py-10 text-slate-300">
                                    <Building2 size={20} className="mx-auto mb-1.5 opacity-20" />
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">لا توجد تسعيرات</span>
                                </td>
                            </tr>
                        ) : (
                            prices.map((price) => (
