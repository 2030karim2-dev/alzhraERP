...
const InventoryStats: React.FC<InventoryStatsProps> = ({ stats }) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
      {cards.map((card, i) => (
        <div key={i} className="bg-[var(--app-surface)] p-3 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-3 transition-all hover:shadow-md group">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">{card.title}</span>
