    ...
    () => [
      {
        ...
        accessor: (row: TransferHistoryRow) => (
          <span className="font-mono font-bold text-gray-400">{row.transfer_number || '-'}</span>
        ),
        width: 'w-24',
      },
      {
        ...
        accessor: (row: TransferHistoryRow) => (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-black text-[11px] text-rose-600 dark:text-rose-400 uppercase tracking-tight">
              {row.from_warehouse?.name_ar || row.from_warehouse_name || '-'}
            </span>
          </div>
        ),
      ...
      },
      {
        ...
        accessor: (row: TransferHistoryRow) => (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-black text-[11px] text-emerald-600 dark:text-emerald-400 uppercase tracking-tight">
              {row.to_warehouse?.name_ar || row.to_warehouse_name || '-'}
            </span>
          </div>
        ),
      ...
      },
      {
        header: 'الأصناف',
        accessor: (row: TransferHistoryRow) => {
          const productNames = (row.items || []).map(
            (i: TransferItemRow) => i.product?.name_ar || i.product?.sku || 'صنف غير معروف'
          );
    ...
    return (
      <div className="space-y-3 pb-4">
        {transfers.map((t: TransferHistoryRow) => (
          <TransferMobileCard key={t.id} transfer={t} />
        ))}
      </div>
    );
  }
