...
import type { ProductCrossReference } from '../types';
...
export const crossReferenceService = {
  ...
  addCrossReference: async (
    ...
    if (error) {
      logger.error(
        ...
        'CrossReferenceService: addCrossReference failed',
        error
      );
      throw error;
    }
  ...
  getCrossReferences: async (
    ...
    if (error) {
      logger.error(
        'crossReferenceService',
        'CrossReferenceService: getCrossReferences failed',
        error
      );
      throw error;
    }
  ...
  deleteCrossReference: async (id: string): Promise<void> => {
    const { error } = await supabase.from('product_cross_references').delete().eq('id', id);

    if (error) {
      logger.error(
        'crossReferenceService',
        'CrossReferenceService: deleteCrossReference failed',
        error
      );
      throw error;
    }
