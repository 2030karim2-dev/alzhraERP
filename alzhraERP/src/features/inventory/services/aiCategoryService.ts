...
export const aiCategoryService = {
  ...
  getUncategorizedProducts: async (companyId: string, limitNum = 100) => {
    ...
    let finalQuery;
    if (generalCat?.id) {
      finalQuery = query.or(`category_id.is.null,category_id.eq.${generalCat.id}`);
    } else {
  ...
  classifyBatch: async (
    ...
    const response = await generateAIContent(prompt, CATEGORY_CLASSIFICATION_PROMPT, {
      jsonMode: true,
    });
  ...
  applyChanges: async (companyId: string, results: ClassificationResult[]) => {
    ...
    const newNamesRequested = [
      ...new Set(results.filter(r => r.is_new_category).map(r => r.suggested_category_name)),
    ];
    ...
    for (const catName of newNamesRequested) {
      ...
      if (!categoryMap[catName]) {
        ...
        if (error) {
          ...
          throw new Error(
            `فشل إنشاء القسم الجديد "${catName}". قد يكون القسم موجوداً بالفعل باسم مشابه.`
          );
        }
        if (data) {
    ...
    const results_data = await Promise.all(
      updates.map(u =>
        supabase.from('products').update({ category_id: u.category_id }).eq('id', u.id).select('id')
      )
    );
