...
export const analyticsService = {
  ...
  getLowStockProducts: async (companyId: string, threshold = 10) => {
    ...
    return (
      (data as Array<{
        ...
        name_ar: string;
        sku: string;
        product_stock?: Array<{ quantity?: number }>;
      }>) || []
    )
      .map(p => {
  ...
  getInventoryAnalytics: async (companyId: string, from?: string, to?: string) => {
    // 1. Fetch Sales Data
    const { data: salesData, error } = await inventoryApi.getInventoryAnalytics(
      companyId,
      from,
      to
    );
    ...
    const allProducts = ((productsData as Array<Record<string, unknown>>) || []).map(p => {
      ...
      const totalStock = stockList.reduce(
        (sum: number, s: Record<string, unknown>) => sum + (Number(s.quantity) || 0),
        0
      );
      return {
        id: p.id as string,
    ...
    activeProducts.forEach(product => {
      ...
      if (percentage <= 0.8) product.abcCategory = 'A';
    ...
    const mostActive = [...activeProducts].sort((a, b) => b.qtySold - a.qtySold).slice(0, 10);

    // 2. Most Profitable
    const mostProfitable = [...activeProducts].sort((a, b) => b.profit - a.profit).slice(0, 10);

    // 3. Stagnant Products
    const thresholdDate = new Date();
    thresholdDate.setDate(thresholdDate.getDate() - 90);

    const stagnant = allProducts
      .filter(p => {
