...
interface ReturnsListViewModel {
  localSearchTerm: string;
  setLocalSearchTerm: (value: string) => void;
  showFilters: boolean;
  setShowFilters: (value: boolean) => void;
  filters: ReturnsListFilters;
  setFilters: (value: ReturnsListFilters) => void;
  sortField: SortField;
  setSortField: (value: SortField) => void;
  sortDirection: SortDirection;
  setSortDirection: (value: SortDirection) => void;
  processedReturns: ReturnsListRow[];
  totalAmount: number;
...
const matchesSearch = (row: ReturnsListRow, searchTerm: string): boolean => {
  ...
  return (
    asSearchText(partyName).includes(query) ||
    asSearchText(row.invoice_number).includes(query) ||
    asSearchText(row.return_reason).includes(query)
  );
};
const matchesFilters = (
  ...
  filters: ReturnsListFilters,
  searchTerm: string
): boolean => amountInRange(row, filters) && matchesSearch(row, searchTerm);

const rowDate = (row: ReturnsListRow): number =>
  new Date(row.issue_date ?? row.created_at).getTime();
const rowPartyName = (row: ReturnsListRow): string =>
  row.party === null ? '' : (row.party.name ?? '');
...
const sortRows =
  ...
  (a: ReturnsListRow, b: ReturnsListRow): number => {
...
const processReturns = ({
  ...
  sortField,
  sortDirection,
}: ProcessOptions): ReturnsListRow[] => {
  if (returns === undefined) return [];
  return [...returns]
    .filter(row => matchesFilters(row, filters, searchTerm))
    .sort(sortRows(sortField, sortDirection));
};
...
const exportDate = (row: ReturnsListRow): string =>
  new Date(row.issue_date ?? row.created_at).toLocaleDateString('ar-SA-u-nu-latn');
const exportItems = (row: ReturnsListRow): number =>
  row.invoice_items === null ? 0 : row.invoice_items.length;
const exportAmount = (row: ReturnsListRow): number => {
  const amount = Number(row.total_amount ?? 0);
  const rate = Number(row.exchange_rate ?? 1);
  if (!row.currency_code || row.currency_code === 'SAR') return amount;
  if (row.currency_code === 'YER' && rate > 0) return rate < 1 ? amount * rate : amount / rate;
  return amount;
};
const toExportRow = (row: ReturnsListRow, type: 'sales' | 'purchase'): ExportReturnRow => {
  const names = exportNames(row, type);
  return {
    invoiceNumber: textOrEmpty(row.invoice_number),
    issueDate: exportDate(row),
    ...names,
    referenceInvoice: '',
    returnReason: textOrEmpty(row.return_reason),
    items: exportItems(row),
    totalAmount: exportAmount(row),
    status: row.status ?? 'draft',
    notes: textOrEmpty(row.notes),
  };
};
const initialFilters = (): ReturnsListFilters => ({
  status: '',
  startDate: '',
  endDate: '',
  minAmount: '',
...
export const useReturnsListView = (
  returns: ReturnsListRow[] | undefined,
  type: 'sales' | 'purchase',
  getFilteredAmount?: (rows: ReturnsListRow[]) => number
): ReturnsListViewModel => {
  ...
  const processedReturns = useMemo(
    () =>
      processReturns({ returns, filters, searchTerm: localSearchTerm, sortField, sortDirection }),
    [returns, filters, localSearchTerm, sortField, sortDirection]
  );
  const totalAmount = useMemo(
    () =>
      getFilteredAmount?.(processedReturns) ??
      processedReturns.reduce(
        (sum, row) => sum + Number(row.total_amount ?? 0) * Number(row.exchange_rate ?? 1),
        0
      ),
  ...
  const handleExportExcel = async (): Promise<void> => {
    ...
    await exportReturnsToExcel({
      ...
      summary: {
        ...
        totalAmount,
        averageAmount: totalAmount / processedReturns.length,
  ...
  const hasActiveFilters =
    Object.values(filters).some(value => value !== '') || localSearchTerm !== '';
  return {
    localSearchTerm,
    setLocalSearchTerm,
    showFilters,
