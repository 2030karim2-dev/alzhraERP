...
const InvoiceOptionList: React.FC<InvoiceOptionListProps> = ({ invoices, selectedInvoiceId, onSelect }) => (
    <div className="max-h-96 overflow-y-auto">
        {invoices.length === 0 ? (
            <div className="p-4 text-center text-sm text-gray-400">لا توجد فواتير مطابقة</div>
        ) : (
...
const InvoiceSelector: React.FC<InvoiceSelectorProps> = ({
    ...
    const filteredInvoices = useMemo(() => {
        ...
        return invoices.filter((invoice): boolean => {
            ...
            const matchItems = invoice.invoice_items?.some((item): boolean =>
