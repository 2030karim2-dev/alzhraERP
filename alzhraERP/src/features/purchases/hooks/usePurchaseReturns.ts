...
const hasCompanyId = (companyId: string | null | undefined): companyId is string => typeof companyId === 'string' && companyId.length > 0;
...
export const usePurchaseReturns = (): UseQueryResult<PurchaseListRow[]> => {
    ...
    return useQuery({
        ...
        queryFn: () => hasCompanyId(companyId) ? purchasesService.getPurchaseReturns(companyId) : Promise.resolve([]),
        enabled: hasCompanyId(companyId),
    });
};

export const usePurchaseReturnsStats = (): UseQueryResult<{ returnCount: number; totalReturns: number; pendingCount: number } | null> => {
    const { user } = useAuthStore();
    const companyId = user?.company_id;
    return useQuery({
        queryKey: ['purchase_returns_stats', companyId],
        queryFn: () => hasCompanyId(companyId) ? purchasesService.getPurchaseReturnsStats(companyId) : Promise.resolve(null),
        enabled: hasCompanyId(companyId),
    });
};
...
export const useCreatePurchaseReturn = (): UseMutationResult<unknown, Error, CreatePurchaseDTO> => {
    ...
    return useMutation({
        mutationFn: async (data: CreatePurchaseDTO) => {
            ...
            if (!hasCompanyId(companyId) || !hasCompanyId(userId)) throw new Error('Missing auth context');
...
export const usePurchaseInvoicesForReturn = (supplierId: string | null): UseQueryResult<PurchaseListRow[] | null> => {
    const { user } = useAuthStore();
    const companyId = user?.company_id;
    return useQuery({
