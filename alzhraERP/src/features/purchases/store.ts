...
type PurchaseActions = Pick<
  PurchaseState,
  'initializeItems' | 'bulkLoadItems' | 'updateItem' | 'setProductForRow' | 'addItem' | 'removeItem'
>;
type PurchaseCalculationActions = Pick<PurchaseState, 'calculateTotals'>;
type PurchaseMetadataActions = Pick<
  PurchaseState,
  'setSupplier' | 'setMetadata' | 'toggleColumn' | 'resetCart'
>;
...
const mapImportedItem = (item: Record<string, unknown>): PurchaseInvoiceItem => ({
  id: crypto.randomUUID(),
  productId: readString(item.productId, 'imported'),
  ...
  costPrice: readNumber(item.unitPrice ?? item.costPrice),
  discount: 0,
});
...
const updateInvoiceItem = (
  ...
  switch (field) {
    ...
    case 'sku':
      return { ...item, sku: String(value) };
    case 'partNumber':
      return { ...item, partNumber: String(value) };
    case 'brand':
      return { ...item, brand: String(value) };
    case 'name':
      return { ...item, name: String(value) };
    case 'quantity':
      return { ...item, quantity: Number(value) };
    case 'costPrice':
      return { ...item, costPrice: Number(value) };
    case 'discount':
...
type PurchaseItemActions = Pick<
  PurchaseActions,
  'initializeItems' | 'bulkLoadItems' | 'updateItem'
>;
  ...
  updateItem: (index, field, value) => {
    set(state => ({
      items: state.items.map((item, itemIndex) =>
        itemIndex === index ? updateInvoiceItem(item, field, value) : item
      ),
    }));
    get().calculateTotals();
  },
...
const createProductActions = (
  set: PurchaseStoreSet,
  get: PurchaseStoreGet
): Pick<PurchaseActions, 'setProductForRow' | 'addItem' | 'removeItem'> => ({
    ...
    set(state => {
      ...
      if (state.currency !== 'SAR') {
        try {
          ...
          logger.error('store', 'PurchaseStore: Invalid exchange rate for setProductForRow', {
            rate,
...
const createMetadataActions = (
  set: PurchaseStoreSet,
  get: PurchaseStoreGet
): PurchaseMetadataActions => ({
...
export const usePurchaseStore = create<PurchaseState>()(
  persist(
    ...
    {
      name: STORAGE_KEYS.PURCHASES_DRAFT,
      partialize: state => ({
        items: state.items,
