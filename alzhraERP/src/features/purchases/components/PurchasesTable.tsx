...
const totalColumn = { header: 'إجمالي (الأساسي)', accessor: (row: PurchaseTableRow): React.ReactElement => <div className="flex flex-col items-end"><span dir="ltr" className={`font-bold font-mono leading-none ${row.type === 'purchase_return' ? 'text-rose-600' : 'text-emerald-600'}`}>{formatCurrency(row.total_amount, row.currency_code ?? undefined)}</span>{hasForeignCurrency(row) && <span dir="ltr" className="text-xs font-bold text-blue-500 mt-1">{formatCurrency(getBaseAmount(row))}</span>}</div>, className: 'text-left' };
