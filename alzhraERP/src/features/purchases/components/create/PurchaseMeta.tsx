...
const MetaBlock: React.FC<MetaBlockProps> = ({ label, value, icon: Icon, isSelect, options, field, colorClass, type = 'text', onChange }) => <div className="relative flex-1 bg-white dark:bg-slate-900 border border-blue-100 dark:border-slate-800 p-2 max-md:p-0.5 flex flex-col group hover:bg-blue-50/40 transition-colors"><div className="flex items-center gap-1 max-md:gap-0.5 mb-1 max-md:mb-0"><Icon size={10} className={colorClass ?? 'text-blue-500'} /><span className="text-[8px] max-md:text-[5px] font-bold text-blue-400 dark:text-blue-600 uppercase tracking-widest leading-none">{label}</span></div>{isSelect === true ? <MetaSelect value={value} onChange={selected => { onChange(field, selected); }} options={options ?? []} placeholder={field === 'cashboxId' ? 'اختر الصندوق...' : field === 'warehouseId' ? 'اختر المستودع...' : 'اختر...'} /> : type === 'date' || type === 'input' ? <input type={type} value={value} onChange={event => { onChange(field, event.target.value); }} className="bg-transparent text-[11px] max-md:text-[7px] font-bold outline-none text-blue-900 dark:text-white text-right font-mono leading-none min-h-3" /> : <span className="text-[11px] max-md:text-[8px] font-bold text-blue-900 dark:text-gray-100 font-mono leading-none">{value}</span>}</div>;
...
const SupplierHeader: React.FC<SupplierHeaderProps> = ({ supplier, query, isOpen, suppliers, setSupplier, setQuery, setIsOpen }) => <div className="relative border-b-2 border-blue-100 dark:border-slate-800">{supplier ? <div className="bg-blue-700 px-3 max-md:px-1.5 py-1.5 max-md:py-0.5 flex items-center justify-between text-white"><div className="flex items-center gap-2"><User size={12} className="text-blue-200" /><span className="text-[11px] max-md:text-[8px] font-bold uppercase">المورد: {supplier.name}</span></div><button type="button" onClick={() => { setSupplier(null); }} className="p-1 hover:bg-white/10 rounded"><X size={14} /></button></div> : <div className="relative"><label htmlFor="purchase-supplier-search" className="sr-only">بحث عن المورد</label><input id="purchase-supplier-search" type="text" value={query} onChange={event => { setQuery(event.target.value); setIsOpen(true); }} placeholder="ابحث عن المورد (Search Supplier)..." className="w-full px-10 max-md:px-6 py-2.5 max-md:py-0.5 bg-white dark:bg-slate-800 text-[10px] max-md:text-[7px] font-bold outline-none text-blue-600 placeholder:text-blue-200" /><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-300" size={14} />{isOpen && query.length > 1 && <div className="absolute z-50 w-full top-full bg-white dark:bg-slate-900 border-2 border-blue-600 shadow-2xl">{suppliers.map(item => <button type="button" key={item.id} onClick={() => { setSupplier(item); setIsOpen(false); }} className="w-full p-3 max-md:p-1.5 hover:bg-blue-600 hover:text-white cursor-pointer text-[11px] max-md:text-[9px] font-bold border-b dark:border-slate-800 text-right">{item.name}</button>)}</div>}</div>}</div>;

interface ExchangeRateBlockProps { exchangeRate: number; isDivide: boolean; onChange: (value: number) => void; }
const ExchangeRateBlock: React.FC<ExchangeRateBlockProps> = ({ exchangeRate, isDivide, onChange }) => <div className="bg-white dark:bg-slate-900 border border-blue-100 dark:border-slate-800 p-2 max-md:p-0.5 flex flex-col group hover:bg-blue-50/40 transition-colors"><div className="flex items-center gap-1 max-md:gap-0.5 mb-1 max-md:mb-0"><Coins size={10} className="text-amber-500" /><span className="text-[8px] font-bold text-amber-500 uppercase tracking-widest">سعر الصرف {isDivide ? '(÷)' : '(×)'}</span></div><input type="number" step="0.00001" value={exchangeRate ? (isDivide ? parseFloat((1 / exchangeRate).toFixed(5)) : exchangeRate) : ''} onChange={event => { const value = parseFloat(event.target.value); if (value) onChange(isDivide ? 1 / value : value); }} className="bg-transparent text-[11px] max-md:text-[7px] font-bold outline-none text-amber-600 dark:text-amber-400 text-right font-mono leading-none" /></div>;
