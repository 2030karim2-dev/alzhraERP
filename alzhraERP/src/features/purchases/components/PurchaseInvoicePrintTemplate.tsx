...
const printStyles = `
    ...
    .purchases-print-box { font-family: 'Arial', 'Segoe UI', sans-serif; color: #000; font-variant-numeric: tabular-nums; }
    .grid-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 13px; }
    .grid-table th, .grid-table td { border: 1px solid #D3D3D3; padding: 8px; }
    .grid-table th { background-color: #1F4E78; color: #FFFFFF; font-weight: bold; text-align: center; }
    .grid-table tr:nth-child(even) td { background-color: #FAFAFA; }
    ...
    .meta-box { border: 1px solid #1F4E78; background-color: #FAFAFA; padding: 10px; border-radius: 4px; display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px; font-size: 13px; }
...
const ItemsTable = ({ items }: { items: PurchasePrintItem[] | null }): React.ReactElement => (
    <table className="grid-table">
        <thead><tr><th style={{ width: '5%' }}>#</th><th style={{ width: '40%' }}>تفاصيل الصنف</th><th style={{ width: '15%' }}>الكمية</th><th style={{ width: '20%' }}>سعر الوحدة</th><th style={{ width: '20%' }}>الإجمالي</th></tr></thead>
        <tbody>
            {items?.map((item, index) => (
                <tr key={item.id}>
                    <td className="text-center">{index + 1}</td>
                    <td><div className="font-bold">{item.description ?? 'بدون وصف'}</div>{item.product?.sku !== null && item.product?.sku !== undefined && <div className="text-xs text-gray-500">SKU: {item.product.sku}</div>}</td>
            ...
            {(items === null || items.length === 0) && <tr><td colSpan={5} className="text-center py-4">لا توجد أصناف</td></tr>}
        </tbody>
    </table>
);

const Totals = ({ invoice }: { invoice: PurchasePrintInvoice }): React.ReactElement => (
    <div className="flex justify-end mt-4"><div className="w-1/2 border border-[#D3D3D3] border-collapse">
        <div className="flex justify-between p-2 border-b border-[#D3D3D3]"><span>المجموع الفرعي</span><span dir="ltr">{formatCurrency(invoice.subtotal ?? 0)}</span></div>
        <div className="flex justify-between p-2 bg-[#EBF1DE] text-[#1F4E78] font-bold text-lg"><span>الإجمالي النهائي</span><span dir="ltr">{formatCurrency(invoice.total_amount)}</span></div>
    </div></div>
);

const Signatures = (): React.ReactElement => (
    <div className="flex justify-between mt-16 pt-8 border-t border-gray-400">
        {['المشتريات', 'الحسابات', 'المدير العام'].map(label => <div className="text-center w-1/3" key={label}><p className="font-bold mb-8">{label}</p><div className="border-t border-black w-24 mx-auto" /></div>)}
    </div>
);

const PurchaseInvoicePrintTemplate = React.forwardRef<HTMLDivElement, PurchaseInvoicePrintTemplateProps>(({ invoice }, ref) => {
    ...
    return <div ref={ref} className="max-w-4xl mx-auto bg-white text-black font-sans print:p-0" dir="rtl">
        ...
        <div className="purchases-print-box p-8">
            ...
            {invoice.notes !== null && invoice.notes !== '' && <div className="mt-6 border border-gray-300 p-4 bg-gray-50"><strong>ملاحظات:</strong><p className="mt-2 text-sm">{invoice.notes}</p></div>}
            <Signatures />
        </div>
    </div>;
});
