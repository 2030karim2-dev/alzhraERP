...
const PurchaseDetailsModal: React.FC<PurchaseDetailsModalProps> = ({ invoiceId, onClose }) => {
    ...
    return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 overflow-y-auto animate-in fade-in duration-200"><div className={cn('bg-[var(--app-surface)] rounded-[2rem] shadow-2xl w-full my-auto flex flex-col max-h-[90vh] border border-gray-100 transition-all duration-300', getSizeClass(size), size === 'full' ? 'max-h-[98vh]' : '')}>
