...
export const purchaseQuotationsApi = {
  getQuotations: async (companyId: string) =>
    supabase
      .from('quotations')
      .select(
        'id, quotation_number, type, status, party_id, issue_date, valid_until, total_amount, currency_code, rfq_group_id, created_at, party:party_id(name), quotation_items(id, product_id, description, quantity, unit_price, total, product:products(size, part_number, sku, brand, name_ar))'
      )
  ...
  getQuotationDetails: async (quotationId: string) =>
    supabase
      .from('quotations')
      .select(
        '*, party:party_id(id, name, phone, email), quotation_items(*, product:products(name_ar, sku, part_number, size, brand))'
      )
      .eq('id', quotationId)
      .single(),
  ...
  getComparisonData: async (rfqGroupId: string, companyId: string) =>
    supabase
      .from('quotations')
      .select(
        'id, quotation_number, status, total_amount, currency_code, exchange_rate, delivery_terms, payment_terms, party:party_id(name), quotation_items(id, product_id, description, quantity, unit_price, total, product:products(size, part_number, sku, brand, name_ar))'
      )
