...
export const migrateCashboxBalances = async (companyId: string, userId: string) => {
  ...
  if (err2 || !sarCashbox)
    throw new Error('لم يتم العثور على صندوق الكاش السعودي. قم بتقسيم الصندوق أولاً.');
  ...
  const mainBalance =
    (balanceRows ?? []).find(row => row.account_id === mainCashbox.id)?.balance ?? 0;
  if (mainBalance === 0) {
