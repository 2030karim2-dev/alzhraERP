...
const TreasurySummaryStats: React.FC<Props> = ({ accountId, dateRange }) => {
  ...
  const isForeign = Boolean(currencyCode && currencyCode !== 'SAR');
  ...
  const stats = useMemo(() => {
    if (!ledger) {
      return {
        ...
        totalCredit: 0,
        foreignTotalDebit: 0,
        foreignTotalCredit: 0,
      };
    }

    let totalDebit = 0;
    let totalCredit = 0;
    let foreignTotalDebit = 0;
    let foreignTotalCredit = 0;
    ...
    for (const item of ledger) {
      ...
      const foreignLineDebit = isForeign
        ? item.foreign_amount && itemDebit > 0
          ? item.foreign_amount
          : itemDebit * (item.exchange_rate && item.exchange_rate > 1 ? item.exchange_rate : 410)
        : itemDebit;
      const foreignLineCredit = isForeign
        ? item.foreign_amount && itemCredit > 0
          ? item.foreign_amount
          : itemCredit * (item.exchange_rate && item.exchange_rate > 1 ? item.exchange_rate : 410)
        : itemCredit;
      ...
      if (isOutflowReversal) {
        ...
        totalCredit -= itemDebit;
        foreignTotalCredit -= foreignLineDebit;
        ...
        totalDebit -= itemCredit;
        foreignTotalDebit -= foreignLineCredit;
      } else {
        totalDebit += itemDebit;
        totalCredit += itemCredit;
        foreignTotalDebit += foreignLineDebit;
        foreignTotalCredit += foreignLineCredit;
      }
    }

    const lastEntry = ledger[ledger.length - 1];
    const currentBalance = lastEntry ? lastEntry.balance : 0;
    const currentForeignBalance = lastEntry ? (lastEntry.foreign_balance ?? lastEntry.balance) : 0;
    ...
    return {
      ...
      foreignTotalDebit: Math.max(0, foreignTotalDebit),
      foreignTotalCredit: Math.max(0, foreignTotalCredit),
    };
  }, [ledger, isForeign]);
  ...
  return (
    <div className="mb-6 grid grid-cols-1 gap-4 max-md:mb-3 max-md:gap-3 md:grid-cols-3">
      <StatCard
        ...
        value={
          isForeign
            ? formatCurrency(Math.abs(stats.foreignBalance), currencyCode)
            : formatCurrency(Math.abs(stats.balance), 'SAR')
        }
        subtext={
          isForeign
            ? `${isCreditBalance ? 'رصيد دائن (مكشوف)' : 'رصيد مدين (متوفر)'} • يعادل ${formatCurrency(Math.abs(stats.balance), 'SAR')}`
            : isCreditBalance
              ? 'رصيد دائن (مكشوف)'
              : 'رصيد مدين (متوفر)'
        }
      ...
      <StatCard
        ...
        value={
          isForeign
            ? formatCurrency(stats.foreignTotalDebit, currencyCode)
            : formatCurrency(stats.totalDebit, 'SAR')
        }
        subtext={
          isForeign
            ? `حركات الإيداع والقبض • يعادل ${formatCurrency(stats.totalDebit, 'SAR')}`
            : 'حركات الإيداع والقبض'
        }
        icon={TrendingUp}
      ...
      <StatCard
        ...
        value={
          isForeign
            ? formatCurrency(stats.foreignTotalCredit, currencyCode)
            : formatCurrency(stats.totalCredit, 'SAR')
        }
        subtext={
          isForeign
            ? `حركات الصرف والتحويل • يعادل ${formatCurrency(stats.totalCredit, 'SAR')}`
            : 'حركات الصرف والتحويل'
        }
        icon={TrendingDown}
