    ...
    return (
        <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-none shadow-sm flex flex-col">
            <div className={` max-md:p-3 border-b border-gray-100 dark:border-slate-800 flex items-center  max-md:gap-2 ${currentTheme.header}`}>
                <div className={` max-md:p-1.5 rounded-none shadow-sm ${currentTheme.icon}`}><Icon size={14} /></div>
                <h3 className="text-[10px] font-bold uppercase tracking-widest">{title}</h3>
            </div>
            <div className="flex-1  max-md:p-2 space-y-px">
                {data.map((item: any) => (
                    <div key={item.code} className="flex justify-between  max-md:p-2 hover:bg-gray-50/50 dark:hover:bg-slate-800/20">
...
const IncomeStatement: React.FC<Props> = ({ dateRange }) => {
    ...
    if (!financials) return <div className="p-8 max-md:p-4 text-center text-gray-500 dark:text-slate-500">لا توجد بيانات مالية متاحة لهذه الفترة</div>;
    ...
    return (
        <div className="max-w-none mx-auto space-y-4 pb-12 print-area animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800  max-md:p-4 rounded-none text-center shadow-sm flex justify-between items-center">
                ...
                <div className="flex items-center  max-md:gap-2 bg-gray-50 dark:bg-slate-800 px-3 py-1.5 rounded-none border border-gray-100 dark:border-slate-700 text-xs text-gray-600 dark:text-slate-400">
                    <Calendar size={12} />
                ...
                <div className="flex items-center  max-md:gap-3">
                    ...
                    />
                    <div className=" max-md:p-2 bg-slate-900 text-white rounded-none shadow-inner">
                        <FileText size={20} />
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2  max-md:gap-4 items-start">
            ...
            {/* Net Income Summary */}
            <div className={cn(
                "relative overflow-hidden rounded-none shadow-xl border-2 p-6 max-md:p-3 flex flex-col md:flex-row justify-between items-center  max-md:gap-4 transition-all hover:scale-[1.01]",
                netIncome >= 0
                    ? 'bg-slate-900 border-emerald-500'
                    : 'bg-red-50 dark:bg-red-950/20 border-red-300 dark:border-red-900/50'
            )}>
                <div className="flex items-center  max-md:gap-3">
                    <div className={cn(" max-md:p-3 rounded-none", netIncome >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-200 dark:bg-red-900/30 text-red-600 dark:text-red-400')}>
                        <DollarSign size={24} />
                    </div>
                    <div>
                ...
                <div dir="ltr" className={cn("text-4xl max-md:text-2xl font-mono font-bold tracking-tight", netIncome >= 0 ? 'text-emerald-400' : 'text-red-600 dark:text-red-400')}>
