...
const layouts: Array<{ key: POSLayout; label: string; icon: React.ReactNode; desc: string }> = [
  {
    key: 'classic',
    label: 'كلاسيكي',
    icon: <LayoutGrid size={14} />,
    desc: 'قائمة منتجات + سلة الشراء',
  },
  { key: 'compact', label: 'مدمج', icon: <Columns size={14} />, desc: 'تخطيط عمودي موفر للمساحة' },
  {
    ...
    desc: 'تخطيط RTL محسّن للغة العربية',
  },
  {
    key: 'fullscreen',
    label: 'شاشة كاملة',
    icon: <Monitor size={14} />,
    desc: 'وضع ملء الشاشة للشاشات الكبيرة',
  },
];

const POSLayoutSwitcher: React.FC<POSLayoutSwitcherProps> = ({ current, onChange, className }) => {
  return (
    <div className={cn('flex items-center gap-1 rounded-xl bg-[var(--app-bg)] p-1', className)}>
      {layouts.map(layout => (
        <button
          key={layout.key}
          onClick={() => {
          ...
          className={cn(
            'flex items-center gap-2 rounded-lg px-3 py-2 text-[10px] font-bold transition-all duration-200',
            current === layout.key
              ? 'border border-[var(--app-border)] bg-[var(--app-surface)] text-[var(--accent)] shadow-sm'
