...
export type SortMode =
  'relevance' | 'price_asc' | 'price_desc' | 'stock_asc' | 'stock_desc' | 'name' | 'popular';
...
export function usePOSSearch(options: UsePOSSearchOptions = {}) {
  ...
  const searchQuery = useQuery({
    ...
    queryFn: () => posSearchService.search(companyId, debouncedQuery, filters, limit),
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      switch (e.key) {
        case 'ArrowDown':
          e.preventDefault();
          setSelectedIndex(prev => (prev < sortedResults.length - 1 ? prev + 1 : 0));
          break;
        case 'ArrowUp':
          e.preventDefault();
          setSelectedIndex(prev => (prev > 0 ? prev - 1 : sortedResults.length - 1));
          break;
        case 'Enter':
          e.preventDefault();
  ...
  return {
    ...
    // Cache control
    invalidateCache: () => {
      posSearchService.invalidateCache();
    },
  };
}
