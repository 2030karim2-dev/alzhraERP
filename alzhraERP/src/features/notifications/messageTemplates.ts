...
import { buildWhatsappHeader, CompanyHeaderSource } from '@/core/utils/whatsappHeader';
import { WhatsappHeaderConfig } from '@/core/types/documentHeader';
...
export function buildTemplatedMessage(
  ...
  data: any,
  headerConfig?: WhatsappHeaderConfig,
  company?: CompanyHeaderSource
): string {
  const generator = messageTemplates[event];
  if (!generator) return '';

  const body = generator(data);
  if (!headerConfig || !company) return body;

  const header = buildWhatsappHeader(headerConfig, company);
  return `${header}${body}`;
}
