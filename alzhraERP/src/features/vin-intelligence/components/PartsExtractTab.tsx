...
import { PackagePlus, Sparkles, ClipboardPaste } from 'lucide-react';
...
export const PartsExtractTab: React.FC<PartsExtractTabProps> = ({
  ...
  const handleApplyBulkParts = (): void => {
    if (!rawBulkPartsText.trim()) return;
    const parsed = parseBulkPartsText(
      rawBulkPartsText,
      vehicle,
      customVehicleTemplate.trim() || undefined
    );
    if (parsed.length > 0) {
    ...
    <div className="font-cairo space-y-4">
      ...
      <div className="space-y-2.5 rounded-2xl border border-blue-200/80 bg-blue-50/40 p-3.5 shadow-sm dark:border-blue-900/50 dark:bg-blue-950/20">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          >
            {isBulkPasteOpen ? 'إخفاء مربع اللصق ▲' : 'إظهار مربع اللصق السريع ▼'}
          </button>
        </div>
        ...
        {isBulkPasteOpen && (
          <div className="space-y-2 border-t border-blue-200/60 pt-2 dark:border-blue-900/40">
            <textarea
              rows={4}
              value={rawBulkPartsText}
              onChange={e => {
                setRawBulkPartsText(e.target.value);
              }}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    if (clip) setRawBulkPartsText(clip);
                  } catch {
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="sm"
                  variant="primary"
                  onClick={handleApplyBulkParts}
                  disabled={!rawBulkPartsText.trim()}
