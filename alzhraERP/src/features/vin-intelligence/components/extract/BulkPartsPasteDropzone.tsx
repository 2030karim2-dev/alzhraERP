...
const DropzoneHeader: React.FC<DropzoneHeaderProps> = ({ isOpen, onToggle }) => (
...
interface BulkPasteActionsProps {
  rawText: string;
  onTextChange: (text: string) => void;
  onApply: () => void;
...
const BulkPasteActions: React.FC<BulkPasteActionsProps> = ({
  rawText,
  onTextChange,
  onApply,
  onPasteClipboard,
...
interface BulkPasteEditorProps {
  rawText: string;
  onTextChange: (text: string) => void;
  onApply: () => void;
  onPasteClipboard: () => void;
...
const BulkPasteEditor: React.FC<BulkPasteEditorProps> = ({
  rawText,
  onTextChange,
  onApply,
  onPasteClipboard,
...
export const BulkPartsPasteDropzone: React.FC<BulkPartsPasteDropzoneProps> = ({
  ...
  return (
    <div className="space-y-2.5 rounded-2xl border border-blue-200/80 bg-blue-50/40 p-3.5 shadow-sm dark:border-blue-900/50 dark:bg-blue-950/20">
      <DropzoneHeader isOpen={isOpen} onToggle={onToggle} />
      {isOpen && (
        <BulkPasteEditor
          rawText={rawText}
          onTextChange={onTextChange}
