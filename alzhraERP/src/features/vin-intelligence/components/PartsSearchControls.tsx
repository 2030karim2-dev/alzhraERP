...
import { Globe, Search, ExternalLink, Plus, Layers } from 'lucide-react';
...
const SearchHeaderRow: React.FC<SearchHeaderRowProps> = ({ selectedCatalogId, onCatalogChange }) => (
...
const SearchInputRow: React.FC<SearchInputRowProps> = ({ searchQuery, onSearchQueryChange, isSearching, onSearch, selectedCatalogId, vehicle, onAddRow, onAddMultipleRows }) => (
  <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
    ...
    <div className="lg:col-span-4 flex items-center gap-2">
      ...
      <Button
        ...
        variant="outline"
        onClick={() => {
          openCatalogSearch(selectedCatalogId, searchQuery || (vehicle?.vinPrefix ?? ''));
        }}
    ...
    <div className="lg:col-span-3 flex items-center justify-end gap-2">
      <Button size="sm" variant="primary" onClick={onAddRow} className="bg-blue-600 hover:bg-blue-700 font-bold text-xs rounded-xl shadow-sm py-2">
        <Plus size={14} className="ml-1" /> سطر جديد
      </Button>
      <Button size="sm" variant="outline" onClick={() => { onAddMultipleRows(5); }} className="font-bold text-xs rounded-xl py-2">
...
interface QuickCatalogsLauncherProps {
  searchQuery: string;
  onCatalogChange: (id: string) => void;
  vehicle: VehicleInfo | null;
}

const QuickCatalogsLauncher: React.FC<QuickCatalogsLauncherProps> = ({ searchQuery, onCatalogChange, vehicle }) => (
  <div className="pt-2 flex flex-wrap items-center gap-1.5 border-t border-slate-100 dark:border-slate-800">
    ...
    {vehicle?.vinPrefix != null && vehicle.vinPrefix !== '' && (
      <button
        type="button"
        onClick={() => {
          openCatalogVinSearch('partsouq', vehicle.vinPrefix ?? '');
        }}
...
export const PartsSearchControls: React.FC<PartsSearchControlsProps> = ({ searchQuery, onSearchQueryChange, selectedCatalogId, onCatalogChange, isSearching, onSearch, vehicle, onAddRow, onAddMultipleRows }) => (
  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
    <SearchHeaderRow selectedCatalogId={selectedCatalogId} onCatalogChange={onCatalogChange} />
    <SearchInputRow searchQuery={searchQuery} onSearchQueryChange={onSearchQueryChange} isSearching={isSearching} onSearch={onSearch} selectedCatalogId={selectedCatalogId} vehicle={vehicle} onAddRow={onAddRow} onAddMultipleRows={onAddMultipleRows} />
    <QuickCatalogsLauncher searchQuery={searchQuery} onCatalogChange={onCatalogChange} vehicle={vehicle} />
  </div>
);
