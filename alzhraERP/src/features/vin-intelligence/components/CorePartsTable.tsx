                  ...
                  <tbody>{catParts.map(part => {
                    ...
                    return (
                      <tr key={part.id} onClick={() => onPartClick?.(part)} className="border-b border-[var(--app-border)] hover:bg-[var(--app-surface-hover)] transition-all cursor-pointer">
                        ...
                        <td className="px-3 py-1.5 text-center">{qty > 0 ? <span className="text-[7px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded uppercase tracking-widest">{t('vin_in_stock')}</span> : part.fitmentStatus === 'NOT_COMPATIBLE' ? <span className="text-[7px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded uppercase tracking-widest">N/A</span> : <span className="text-[7px] font-bold text-rose-600 bg-rose-50 dark:bg-rose-900/20 px-1.5 py-0.5 rounded uppercase tracking-widest">{t('vin_out_of_stock')}</span>}</td>
                      </tr>
                    );
                  })}</tbody>
                </table>
