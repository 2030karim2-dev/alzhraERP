...
export function useDefaultExchangeRates() {
  ...
  useEffect(() => {
    if (hasTriggeredCurrencySync || rates.isLoading || !rates.data || !user?.company_id) return;
    ...
    if (rates.data.length === 0) {
      logger.info(
        ...
        '[Currency] No rates found, triggering initial market sync...'
      );
      refreshRates();
      // Do not return here so fallback rates can be immediately inserted to unblock UI
    }

    const existingRates = (rates.data || []) as Array<{
