...
export const PaymentMethodsChart: React.FC<PaymentMethodsChartProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-[var(--app-surface)] p-6 dark:border-slate-800 max-md:p-3">
      <h4 className="mb-6 flex items-center gap-2 text-base font-bold text-slate-800 dark:text-white max-md:gap-2">
        <Wallet size={18} className="text-purple-600" />
        {t.payment_methods}
      </h4>
      <div ref={containerRef} className="group relative h-56 w-full sm:h-64" dir="ltr">
        {isMounted && (
              ...
              <Tooltip
                content={({ active, payload }: any) => {
                  if (active && payload?.length) {
                    ...
                    return (
                      <div className="rounded-2xl border border-slate-200/50 bg-white/90 p-3 shadow-2xl backdrop-blur-xl transition-all duration-300 dark:border-slate-700/50 dark:bg-slate-900/90 max-md:p-3">
                        <div className="mb-2 flex items-center gap-2 max-md:gap-2">
                          <div className={cn('rounded-lg p-1 max-md:p-1.5', info.bg)}>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="font-mono text-xl font-bold text-slate-800 dark:text-white">
            {formatCurrency(total)}
          </span>
        </div>
      </div>
      <div className="mt-6 grid grid-cols-1 gap-2 max-md:gap-2 sm:grid-cols-2">
        {salesByPaymentMethod.map((method, _index) => {
          ...
          return (
            <div
              key={method.method}
              className="flex items-center justify-between rounded-2xl border border-transparent bg-slate-50 p-3 transition-all duration-300 hover:border-slate-200 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:border-slate-700 dark:hover:bg-slate-800 max-md:p-3"
            >
              <div className="flex items-center gap-2 max-md:gap-2">
                <div className={cn('rounded-xl p-2 max-md:p-2', info.bg)}>
                ...
                <div className="flex flex-col">
                  ...
                  <span className="mt-1 text-[10px] font-bold text-slate-400">{percentage}%</span>
                </div>
              </div>
