/* eslint-disable complexity, max-lines-per-function, security/detect-object-injection, @typescript-eslint/no-base-to-string, @typescript-eslint/restrict-template-expressions, @typescript-eslint/explicit-function-return-type, @typescript-eslint/strict-boolean-expressions */
// ============================================
...
const normalizeAnalytics = (raw: RawRecord | null | undefined): SalesAnalytics | null => {
  ...
  const listField = (key: string, snakeKey: string): RawRecord[] =>
    Array.isArray(raw[key])
      ? (raw[key] as RawRecord[])
      : Array.isArray(raw[snakeKey])
        ? (raw[snakeKey] as RawRecord[])
        : [];
  ...
  return {
    ...
    topProducts: listField('topProducts', 'top_products').map((p: RawRecord, i: number) => ({
      ...
      currentStock: toNum(p.currentStock ?? p.current_stock),
    ...
    salesByPaymentMethod: listField('salesByPaymentMethod', 'sales_by_payment_method').map(
      (m: RawRecord) => ({
...
export const useSalesAnalytics = (params: SalesAnalyticsParams) => {
  ...
  const queryKey = ['sales_analytics', companyId, { startDate, endDate, period }] as const;
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const rawRecord =
        analytics !== null && typeof analytics === 'object' && !Array.isArray(analytics)
