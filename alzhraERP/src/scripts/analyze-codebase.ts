...
issues.directDOMManipulation.slice(0, 5).forEach(f => {
  console.log(`  - ${f}`);
});
...
issues.nakedApiCalls.slice(0, 10).forEach(f => {
  console.log(`  - ${f}`);
});
...
console.log(
  `  (Top 10 list omitted for brevity, but ${issues.anyTypes.length} is a metric to track)`
);
