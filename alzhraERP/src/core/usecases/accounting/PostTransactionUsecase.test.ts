...
describe('PostTransactionUsecase', () => {
  ...
  it('should validate data and post journal entry successfully using real Zod Schema and mapping', async () => {
    ...
    // Assertions
    expect(journalsApi.postJournalEntryRPC).toHaveBeenCalledWith(mockCompanyId, mockUserId, {
  ...
  it('should throw Zod error if validation fails due to unbalanced entries', async () => {
    ...
    // Execute and assert
    await expect(
      PostTransactionUsecase.execute(invalidInputData, mockCompanyId, mockUserId)
    ).rejects.toThrow();
  ...
  it('should throw an error if posting to API fails', async () => {
    ...
    await expect(
      PostTransactionUsecase.execute(mockInputData, mockCompanyId, mockUserId)
  ...
  it('should reject single-line entries (minimum 2 required)', async () => {
    const invalidInput = {
      ...
      lines: [{ account_id: validUUID1, debit_amount: 100, credit_amount: 0 }],
    };

    await expect(
      PostTransactionUsecase.execute(invalidInput, mockCompanyId, mockUserId)
    ).rejects.toThrow();
    expect(journalsApi.postJournalEntryRPC).not.toHaveBeenCalled();
  });
