  ...
  items.forEach((item, index) => {
    if (!item.productId) {
      errors.push({
        ...
        message: `الصنف ${index + 1}: يجب تحديد المنتج`,
      });
    }
    if (!item.quantity || item.quantity <= 0) {
      errors.push({
        field: `items[${index}].quantity`,
        message: `الصنف ${index + 1}: الكمية يجب أن تكون أكبر من صفر`,
      });
    }
    ...
    if (price < 0) {
      errors.push({
        ...
        message: `الصنف ${index + 1}: السعر لا يمكن أن يكون سالباً`,
      });
    }
  });
...
export const validatePurchasePayload = (payload: {
  ...
  const errors = validateInvoiceItems(payload.items.map(i => ({ ...i, unitPrice: i.costPrice })));
