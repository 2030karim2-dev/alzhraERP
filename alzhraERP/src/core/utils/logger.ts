...
class Logger {
  ...
  info(context: string, message: string, data?: unknown): void {
    ...
    const merged = this.merge(data);
    console.info(
      `${LOG_COLORS.info}${this.prefix('info', context)}${RESET_COLOR}`,
      message,
      merged
    );
  }
  ...
  warn(context: string, message: string, data?: unknown): void {
    ...
    const merged = this.merge(data);
    console.warn(
      `${LOG_COLORS.warn}${this.prefix('warn', context)}${RESET_COLOR}`,
      message,
      merged
    );
  }
  ...
  error(context: string, message: string, error?: unknown): void {
    ...
    console.error(
      `${LOG_COLORS.error}${this.prefix('error', context)}${RESET_COLOR}`,
      message,
      merged
    );
  ...
  trace(context: string, step: string, data?: unknown): void {
    ...
    const merged = this.merge(data);
    console.info(
      `${LOG_COLORS.debug}${this.prefix('debug', context)}${RESET_COLOR} [TRACE]`,
      step,
      merged
    );
  }
