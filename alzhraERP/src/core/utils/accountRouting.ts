...
export const routeToChildByCurrency = (
  ...
  if (childAccounts.length > 0) {
    ...
    const matchedChild = childAccounts.find(a => (a.currency_code || 'SAR') === currency);
...
export const resolveStrictPaymentAccount = (
  ...
  if (exactMatch) {
    logger.info(
      'accountRouting',
      `Strict Currency Match: Routed to ${exactMatch.name} (${exactMatch.code}) for currency ${currency}`
    );
  ...
  if (currency === 'YER') {
    const yerAccount = cashAccounts.find(
      a => a.name?.includes('يمن') || a.name?.includes('YER') || a.name?.includes('ريال يمني')
    );
    if (yerAccount) {
      logger.info(
        'accountRouting',
        `Strict Keyword Match: Routed to ${yerAccount.name} (${yerAccount.code}) for currency YER`
      );
      return yerAccount.id;
    }
  } else if (currency === 'SAR') {
    const sarAccount = cashAccounts.find(
      a =>
        a.name?.includes('سعود') ||
        a.name?.includes('SAR') ||
        a.name?.includes('رئيسي') ||
        a.currency_code === 'SAR'
    );
    if (sarAccount) {
      logger.info(
        'accountRouting',
        `Strict Keyword Match: Routed to ${sarAccount.name} (${sarAccount.code}) for currency SAR`
      );
