...
export const useErrorHandler = (
  ...
  const handleError = useCallback(
    (error: unknown, context?: string): AppError => {
      ...
      if (error instanceof AppError) {
        ...
        appError = new AppError(error.message, ErrorCode.UNKNOWN, 500, {
  ...
  const handleErrorAsync = useCallback(
    async function <T>(
      promise: Promise<T>,
      context?: string
...
export const useSafeErrorHandler = () => {
  ...
  const safeHandle = useCallback(
    <T>(error: unknown, defaultValue: T, context?: string): T => {
